...
/** This simply counts the messages received from the given channel.
  *
  * @param config the actor configuration
  */
class MessageCounterActor(val config: Config) extends MesaActor {

  var counter = 0

  /** Simply increases the value of 'counter' by one upon receiving each
    * message.
    *
  ...
  override def onTerminateRaceActor(originator: ActorRef) = {
    println(s"number messages: $counter")
