...
import gov.nasa.mesa.nextgen.verification.monitors.tracecontract.{FlightMsgOrder_Monitor, SingleFlightMsgOrder_Monitor}
...
/**
  * A test suite for gov.nasa.mesa.nextgen.verification.monitors.tracecontract.
  * FlightMsgOrder_Monitor/SingleFlightMsgOrder_Monitor
  */
class SfdpsOrderTCMonitorSpec extends RaceActorSpec with AnyWordSpecLike {
  ...
  "AllFlightSfdpsOrderTCMonitor" must {
    "check if sfdps messages for a flight are received " +
      ...
      monitor.getMonitorResult.numberOfErrors should be(0)
    }
  }
  ...
  "AllFlightSfdpsOrderTCMonitor" must {
    "report an error when sfdps messages for a flight are not " +
      ...
      monitor.getMonitorResult.numberOfErrors should be(1)
    }
  }
