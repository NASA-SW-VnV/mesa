...
//-- main events and messages for mesa-nextgen
...
/** This class is the extended version of FlightState
  *
  * @param id the flight identifier //changes over the course of the flight
  * @param cs the flight call sign
  * @param position the flight position
  ...
  */
class ExtendedFlightState(id: String,
                          cs: String,
                          position: GeoPosition,
                          speed: Speed,
                          heading: Angle,
  ...
  def hasflightPlan: Boolean = fplan.hasFlightPlan

  override def toString = {
    var s1, s2 = ""

    s1 = s"ExtendedFlightState($id,$cs,$position,${speed.toKnots.toInt}kn," +
      s"${heading.toNormalizedDegrees.toInt}°,$date, 0x${status.toHexString}"
...
/** The companion extractor object for ExtendedFlightState. */
object ExtendedFlightState {

  /** A constructor which takes relevant arguments and creates a ExtendedFlightState
    * object.
    *
    ...
    * @param fplan the flight plan information
    * @param equipmentQualifier the equipment qualifier
    * @return an instance of the class ExtendedFlightState.
    */
  def apply(id: String, cs: String, position: GeoPosition,
            speed: Speed, heading: Angle, vr: Speed,
            date: DateTime, status: Int, src: String,
    ...
    new ExtendedFlightState(id, cs, position, speed, heading, vr, date, status, src,
      departurePoint, departureDate, arrivalPoint, arrivalDate, fplan,
      equipmentQualifier)
  }

  /** A constructor which takes relevant arguments and creates an ExtendedFlightState
    * object.
    *
    * Note that this is defined for testing purposes.
    *
    * @param id the flight identifier
    * @param cs the flight call sign
    * @param departurePoint the departure airport
  ...
  def apply(id: String, cs: String,
            ...
            fplan: FlightPlan, equipmentQualifier: String): ExtendedFlightState = {
    new ExtendedFlightState(id, cs, GeoPosition.undefinedPos, Speed.UndefinedSpeed,
  ...
  def unapply(ft: ExtendedFlightState): Option[(String, String, GeoPosition, Speed,
    Angle, Speed, DateTime, Int, String, String, FlightPlan, String)] = {
    Some(ft.id, ft.cs, ft.position, ft.speed, ft.heading, ft.vr, ft.date,
      ft.status, ft.departurePoint, ft.arrivalPoint, ft.fplan,
...
/** This event occurs if the fight crosses inside a specified (e.g. 1.0 NM)
  ...
  *
  * @param flightState a message including flight information
  * @param waypoint a waypoint
  */
case class WaypointVisit(flightState: ExtendedFlightState, waypoint: Waypoint)

/** An event indicating the change of RNAV STAR.
  *
  * @param flightState the new track information assigned to the flight
  */
case class StarChanged(flightState: ExtendedFlightState)
