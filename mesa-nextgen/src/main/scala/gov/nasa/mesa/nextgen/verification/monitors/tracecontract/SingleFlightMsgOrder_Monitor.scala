...
/** This class represents a TraceContract monitor that captures a property that
  * checks if messages for the flight with the call sign 'cs' are received in a
  * chronological order.
  *
  * @param config the configuration
  */
class SingleFlightMsgOrder_Monitor(config: Config)
  extends TraceContractMonitor(config) {

  val cs: String = config.getString("cs")

  property(Symbol("flight_seq_order")) {
    always {
      case FlightState(_, `cs`, _, _, _, _, date1, _) => state {
        case FlightState(_, `cs`, _, _, _, _, date2, _) => date2.isAfter(date1)
      }
    }
  }
  ...
  override def verifyAfterEvent(msg: Any): Unit = {
    msg match {
      case FlightState(_, `cs`, _, _, _, _, _, _) =>
