...
import gov.nasa.mesa.nextgen.core.ExtendedSfdpsTrack
import gov.nasa.race.air.SfdpsTrack
import gov.nasa.race.geo.GeoPosition
import gov.nasa.race.uom.Angle.Degrees
import gov.nasa.race.uom.{DateTime, Length, Speed}

/** This class represents a translator that translates SFDPS MessageCollection
  * messages obtained from SWIM to ExtendedSfdpsTrack objects.
  */
class ExtendedSfdpsTrackGenerator extends SfdpsParser {

  /** Creates an instance of an ExtendedSfdpsTrack object.
    *
  ...
  override def createSfdpsObject(id: String, cs: String, lat: Double,
    ...
    ExtendedSfdpsTrack(id, cs, GeoPosition(Degrees(lat), Degrees(lon), alt), spd,
      Degrees(Math.atan2(vx, vy).toDegrees), vr, date, status, src,
      departurePoint, departureDate, arrivalPoint, arrivalDate, route,
